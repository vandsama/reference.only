  ...
  methods: {
    ...
    deleteBoard() {
      ...
      boardsService
        ...
        .catch((err) => {
          if (err.response) {
            this.errorMsg = `could not remove the board: ${err.response.statusText}`;
          } else if (err.request) {
            this.errorMsg =
              "could not remove the board. no one answered the call";
          } else {
            this.errorMsg =
              "could not remove the board. probably not your fault";
          }
        });
    },
  },
