  ...
  methods: {
    ...
    saveNewBoard() {
      ...
      boardsService
        ...
        .catch((err) => {
          if (err.response) {
            this.errorMsg = `oh man! something went wrong: ${err.response.statusText}`;
          }
          if (err.request) {
            this.errorMsg = "oh man! no one's home when I tried to call";
          } else {
            this.errorMsg =
              "something went totally bad! it's probably not your fault.";
          }
          this.isLoading = false;
        });
    },
